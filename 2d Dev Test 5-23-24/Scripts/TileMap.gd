...
func destroy_tile_at(location : Vector2i):
	if get_cell_atlas_coords(2, location) == Vector2i(4,0):
		spawning(location)
	if get_cell_atlas_coords(2, location) == Vector2i(0,0):
...
func rock_spawning():
	...
	for r in row:
		for c in col:
			...
			global_pos = map_to_local(cell_position)
			if get_cell_tile_data(1, cell_position) != null and get_cell_tile_data(3, cell_position) == null:
				if noiseimage.get_pixelv(global_pos) == Color.WHITE:
					set_cell(2, cell_position,0,Vector2(5,0))
					set_cell(1, cell_position,2,Vector2(5,0))
				elif noiseimage.get_pixelv(global_pos) == Color.BLACK:
					set_cell(2, cell_position,0,Vector2(0,0))
					set_cell(1, cell_position,2,Vector2(0,0))
				elif noiseimage.get_pixelv(global_pos) == Color.RED:
					set_cell(2, cell_position,0,Vector2(4,0))
					set_cell(1, cell_position,2,Vector2(4,0))
	loaded = true
...
func wincon():
	...
	for r in row:
		for c in col:
			...
			var global_pos : Vector2i
			global_pos = map_to_local(cell_position)
			if get_cell_atlas_coords(2, cell_position) == Vector2i(0,0):
				emerald_count += 1
...
func open_level_gate():
	...
	if get_cell_tile_data(2, cell_position) == null and end_spawned == false and get_cell_tile_data(0, cell_position) == null:
		...
		set_cell(3, cell_position, 3, Vector2(0,0))
